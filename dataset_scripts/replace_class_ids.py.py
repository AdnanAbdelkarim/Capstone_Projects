...
# ✅ Create a mapping: "Brand Model" -> Class ID
model_class_mapping = { 
    " ".join(model.split()[:2]): idx for idx, model in enumerate(vehicle_models)
}
...
for label_file in os.listdir(LABELS_DIR):
    ...
    # Extract the first two words from the filename (replacing underscores)
    base_name = os.path.splitext(label_file)[0].replace('_', ' ')
    first_two_words = " ".join(base_name.split()[:2])

    matched_class_id = model_class_mapping.get(first_two_words)

    if matched_class_id is None:
        print(f"⚠️ No match found for: {label_file}. Skipping...")
        continue  # No match found, skip this label
    ...
    for line in lines:
        ...
        if old_class_id != matched_class_id:
            parts[0] = str(matched_class_id)
            labels_updated += 1

        new_lines.append(" ".join(parts) + '\n')

    # ✅ Save updated labels if any changes were made
    if labels_updated > 0:
        with open(label_path, 'w') as f:
            f.writelines(new_lines)

        updated_files += 1
        total_labels_updated += labels_updated
        print(f"✅ Updated {labels_updated} labels in: {label_file}")

# ✅ Final Summary
print("\n🚀 Class ID Update Complete!")
print(f"📂 Total label files processed: {len(os.listdir(LABELS_DIR))}")
print(f"✅ Files modified: {updated_files}")
print(f"🔢 Total class IDs updated: {total_labels_updated}")
